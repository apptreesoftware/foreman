...
7. **Outcome.** Parses the session's structured JSON outcome and applies labels/Status (table below). A session that ends without a valid outcome is retried up to 3 attempts total (`dispatchWithRetry` in `src/dispatch.ts`); if all 3 fail, the issue is labeled `blocked` with a log excerpt and unassigned.
...
- **A validator screenshot is missing from `.playwright-mcp/`.** The screenshot tool sometimes writes the PNG to the worktree root instead of `.playwright-mcp/`; check both locations before concluding a screenshot step failed.
